...
.c-blue{color:#3b82f6;} .c-green{color:#22c55e;} .c-red{color:#ef4444;} .c-yellow{color:#f59e0b;}
...
def download_gdrive(fid, retries=4, timeout=45):
    """Download dari Google Drive dengan 4-lapis bypass confirmation."""
    if not fid:
        return None
...
def download_file(link):
    """Download file dari URL apapun. Return bytes atau None."""
    if not isinstance(link, str) or not link.strip():
        return None
    fid = extract_fid(link)
    if fid:
...
def img_bytes_to_pdf(img_bytes):
    """Konversi gambar (JPG/PNG) ke 1-halaman PDF pakai reportlab."""
    try:
...
# ══════════════════════════════════════════════════════════════════════════════
# THREAD-SAFE WORKER
# Kunci fix: JANGAN akses st.session_state di dalam thread!
# Cache di-pass sebagai plain dict biasa.
# ══════════════════════════════════════════════════════════════════════════════

def _worker(task, cache_snapshot):
    """
    task          : dict {idx, nopol, kuantum, link}
    cache_snapshot: plain dict (BUKAN st.session_state) — aman diakses dari thread
    Return        : dict {idx, nopol, kuantum, link, content}
    """
    link = task['link']
    ct   = cache_snapshot.get(link)   # baca dari plain dict → aman
    if ct is None:
...
def run_bulk_download(disp):
    """
    Jalankan download paralel untuk semua baris di `disp`.
    Return (ok_files: dict, fail_list: list, new_cache: dict)
    """
    # ⚠️ Salin ke plain dict SEBELUM masuk ThreadPoolExecutor
    ...
    prog       = st.progress(0)
    stxt       = st.empty()
    ok_files   = {}   # {filename: bytes}
    new_cache  = {}   # {link: bytes}  — untuk update session_state sesudahnya
    fail_list  = []
    done_n     = 0
    total      = len(tasks)
...
if process:
    if not file1 or not file2:
        ...
        with st.spinner('Memproses data...'):
            ...
            st.session_state.dl_cache       = {}   # reset cache tiap proses baru
...
if st.session_state.result_df is not None:
    ...
    st.markdown(f"""
    <div class="stat-grid">
      <div class="stat-card"><div class="stat-num c-blue">{len(df1_all)}</div>
        <div class="stat-lbl">Total Data File 1</div></div>
      <div class="stat-card"><div class="stat-num c-green">{len(found)}</div>
        <div class="stat-lbl">Surat Jalan Ditemukan</div></div>
      <div class="stat-card"><div class="stat-num c-red">{len(missing)}</div>
        <div class="stat-lbl">Data Tidak Match</div></div>
      <div class="stat-card"><div class="stat-num c-yellow">{len(df2_all)}</div>
        <div class="stat-lbl">Total Data File 2</div></div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if len(found) > 0:
        ...
        with bc1:
            do_zip    = st.button('📦 Download ZIP',   use_container_width=True,
                                  help='Semua file terpisah dalam satu ZIP')
        with bc2:
            do_merge  = st.button('📄 Gabung 1 PDF',   use_container_width=True,
                                  help='Gabungkan semua menjadi 1 file PDF')
        with bc3:
        ...
        if do_preload and len(disp) > 0:
            ...
            if not links_needed:
                ...
                # Update session_state SETELAH thread selesai (aman)
        ...
        for i, row in disp.iterrows():
            ...
            with cols[5]:
                # Ambil dari cache (plain dict) — tidak ada thread di sini, aman
                ...
                if cached:
                    ...
                    if st.button('⬇️ Download', key=f'db_{i}'):
                        with st.spinner(f'Mengunduh {nopol}...'):
                            # Download single file — tidak butuh thread, langsung aman
                            ct = download_file(link)
                        if ct:
        ...
        Lihat tabel di bawah untuk detail penyebabnya.
        </div>
        """, unsafe_allow_html=True)

    # ══════════════════════════════════════════════════════════════════════════
    # BAGIAN 2 — DATA TIDAK MATCH
    # ══════════════════════════════════════════════════════════════════════════
    st.markdown('<div class="section-label">❌ Data Tidak Berhasil Terdeteksi</div>',
                unsafe_allow_html=True)
    ...
    if len(missing) == 0:
        ...
        st.markdown(f'<div class="error-box">❌ <b>{len(missing)} kombinasi tidak ditemukan.</b></div>',
                    unsafe_allow_html=True)

        # Tabel A
        if nopol_diff is not None and not nopol_diff.empty:
            st.markdown(f'<div class="table-card-yellow"><div class="table-title table-title-yellow">'
                        f'⚠️ Tabel A — {len(nopol_diff)} NOPOL ada, KUANTUM tidak cocok'
                        f'</div></div>', unsafe_allow_html=True)
            sd = st.text_input('', placeholder='🔍 Filter Tabel A...', label_visibility='collapsed', key='sd')
            dd = nopol_diff.copy()
            if sd.strip():
            ...
            with _ca:
                st.download_button('📥 Export Tabel A (.csv)',
        ...
        if nopol_miss is not None and not nopol_miss.empty:
            st.markdown(f'<div class="table-card-red"><div class="table-title table-title-red">'
                        f'❌ Tabel B — {len(nopol_miss)} NOPOL tidak ada di File 2'
                        f'</div></div>', unsafe_allow_html=True)
            sm = st.text_input('', placeholder='🔍 Filter Tabel B...', label_visibility='collapsed', key='sm')
            dm = nopol_miss.copy()
            if sm.strip():
        ...
        with st.expander('📋 Tabel C — Semua tidak match (gabungan)', expanded=False):
            ...
            with _cc:
                st.download_button('📥 Export Tabel C (.csv)',
